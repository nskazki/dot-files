...
class AppendFullStop(sublime_plugin.TextCommand):
  def run(self, edit, **args):

    def insert_full_stop(point):
      self.view.insert(edit, point, '.')

    def is_full_stop(point):
    ...
    for region in self.view.sel():
      ...
      if self.view.match_selector(line_end - 1, 'comment'):
        ...
        if not self.view.substr(point).isspace() and not is_full_stop(point):
          insert_full_stop(point + 1)

      elif not is_full_stop(line_end - 1):
        insert_full_stop(line_end)
